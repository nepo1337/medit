...
Terrain::Terrain(float width, float height, int texScale)
{
	...
	this->texH=0;
	this->vaoh=0;
	this->vbohs[0]=this->vbohs[1]=this->vbohs[2]=0;
	
	//upload text
	this->texH = SOIL_load_OGL_texture("terrain//medalpha.png",SOIL_LOAD_AUTO,SOIL_CREATE_NEW_ID,SOIL_FLAG_MIPMAPS |SOIL_FLAG_INVERT_Y | SOIL_FLAG_COMPRESS_TO_DXT|SOIL_FLAG_TEXTURE_REPEATS);
	if(debug)
	{
		if(texH==0)
